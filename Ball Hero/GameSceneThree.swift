...
class GameSceneThree: SKScene, SKPhysicsContactDelegate  {
    ...
    override func didMoveToView(view: SKView) {
        ...
        NSUserDefaults.standardUserDefaults().setObject(3, forKey: "currentLevel")
    ...
    func highScore(score: Double) -> Bool {
        var isHigh = Bool()
        if NSUserDefaults.standardUserDefaults().objectForKey("highScoreLevelThree") != nil {
            let oldScore = NSUserDefaults.standardUserDefaults().objectForKey("highScoreLevelThree") as! Double
            if score > oldScore {
                NSUserDefaults.standardUserDefaults().setObject(score, forKey: "highScoreLevelThree")
                isHigh = true
            } else {
                isHigh = false
            }
